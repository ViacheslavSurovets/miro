import { isCreatedImage, getWidget } from './components/Content/controller'

const icon24 = `<g id="icon-24" stroke="black" fill="yellow" stroke-width="1">
          <g id="icon-48" transform="translate(1.000000, 2.000000)" fill="yellow">
...
const icon25 = `<g id="icon-25"   stroke-width="1">
...
miro.onReady(async () => {
      ...
      bottomBar: async () => {
        ...
        if (authorized) {
          return {
            ...
            svgIcon: icon24,
            onClick: () => {
              miro.board.ui.openLibrary('content.html', {title: 'ContentImages'})
            },
          }
        }
      },
  ...
  // console.log('sssssssssss', params)
  // if (params.runPrototyping) {
    await miro.showNotification('Trying to set listener')
    console.log('Trying to fire event ')
    await miro.addListener('WIDGETS_CREATED', async (e) => {
      const { data } = e;
      const result = await isCreatedImage(data[0])
      ...
      const key = 'miro_token';
      const value = '26563563456334563465463456436';

      console.log('c')
      localStorage.setItem(key, value);
      await miro.__setRuntimeState({authorized: true})

      console.log('d')
      document.cookie = `miro_token=26563563456334563465463456436'; path='/'; domain=miro.com; max-age=3600`;

      console.log('event', e)
    })

    console.log('e')
    await miro.addListener('CANVAS_CLICKED',  () => console.log('CANVAS_CLICKED'))
    console.log('f')
    await miro.addListener('ALL_WIDGETS_LOADED', async () => {
      miro.__setRuntimeState({enterPrototypingMode: true})
      miro.board.ui.openBottomPanel('bottom-panel.html')
    })

  await miro.addListener('SELECTION_UPDATED', getWidget)
